...
SSL_write(SSL *ssl, const void *buf, int num) {
    if (get_SSL_write() == NULL) {
        set_SSL_write(dlsym(RTLD_NEXT, "SSL_write"));
    }
...
SSL_read(SSL *ssl, void *buf, int num) {
    if (get_SSL_read() == NULL) {
        set_SSL_read(dlsym(RTLD_NEXT, "SSL_read"));
    }
...
SSL_get_verify_result(const SSL *ssl) {
    if (get_SSL_get_verify_result() == NULL) {
        set_SSL_get_verify_result(dlsym(RTLD_NEXT, "SSL_get_verify_result"));
    }
...
SSL_CTX_set_verify(SSL_CTX *ctx, int mode, SSL_verify_cb verify) {
    if (get_SSL_CTX_set_verify() == NULL) {
        set_SSL_CTX_set_verify(dlsym(RTLD_NEXT, "SSL_get_verify_result"));
    }
